...
_.extend(Connection.prototype, EventEmitter.prototype, {
     ...
     */
    disconnect: function(clearUnavailableTimeout){
        if (clearUnavailableTimeout === undefined) {
            clearUnavailableTimeout = true;
        }

        this.clearRetryIn();
        if (clearUnavailableTimeout) {
    ...
    handleError: function(code, message){
        ...
        if(code < 4000) {
            // ignore 1007...3999
            // handle 1000 CLOSE_NORMAL, 1001 CLOSE_GOING_AWAY, 1002 CLOSE_PROTOCOL_ERROR,
            // 1003 CLOSE_UNSUPPORTED, 1004 CLOSE_TOO_LARGE, 1005 CLOSE_NO_STATUS, 1006 CLOSE_ABNORMAL
    ...
    retryIn: function(delay, backoff){
        ...
        Pusher.debug('Retrying connection in ' + delay + ' millisecond(s)');

        self.retryInTimeout = setTimeout(function(){
            var clearUnavailableTimeout = false;
            self.disconnect(clearUnavailableTimeout);
            self.retry = true;
            self.connect();
        }, delay);
    },
    ...
    setUnavailableTimeout: function(){
        ...
        // Only clear the timeout when you explicitly disconnect or the connection is established
        // otherwise any connection retry attempts will reset the timer
        if (!self.unavailableTimeout) {
    ...
    updateState: function(newState, data){
        ...
        if(previousState !== newState){
            ...
            if (!(previousState === 'retry' && newState === 'disconnected')) {
